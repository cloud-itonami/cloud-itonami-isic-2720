(ns cellworks.operation
  "OperationActor -- one lithium-ion battery cell/pack manufacturing
  operation = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (Cell Advisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Cell-Safety
  ...
  One graph run = one battery-plant operation (intake -> advise ->
  ...
  decision to a human operator (the battery-plant quality engineer).
  The approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:actuation/ship-cell-batch`/`:actuation/issue-safety-
  certificate` ALWAYS reach this node when the governor is clean -- see
  `cellworks.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [cellworks.cellworksadvisor :as cellworksadvisor]
...
(defn build
  ...
  `cellworks.store/Store`).
  opts:
    :advisor      -- a `cellworks.cellworksadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (cellworksadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Cell Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (cellworksadvisor/-advise advisor store request)]
            {:proposal p :audit [(cellworksadvisor/trace request p)]})))

      ;; Cell-Safety Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
